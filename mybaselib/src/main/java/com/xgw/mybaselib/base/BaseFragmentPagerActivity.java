 ...
 * viewpager+tablayout+fragment依附的activity基类
 */
...
public abstract class BaseFragmentPagerActivity extends BaseActivity {
    ...
    public void initView() {
        ...
        if (isCustomTabLayout()) {
            viewPager.setAdapter(new BaseCustomViewPagerAdapter(getSupportFragmentManager(),getFragments()));
        } else {
            viewPager.setAdapter(new BaseViewPagerAdapter(getSupportFragmentManager(), getTitles(), getFragments()));
        }
     ...
     * 设置tablayout滚动模式
     *
     * @param mode TabLayout.MODE_SCROLLABLE（多余tab可横向滚动出去） TabLayout.MODE_FIXED（不管多少tab都显示在屏幕里，不可滚动，过多可导致文字显示不全）
     */
